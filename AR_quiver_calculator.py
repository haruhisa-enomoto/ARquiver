"""A program to work with AR quivers and compute various objects.
...
class Arrow:
    ...
    def draw(self) -> None:
        ...
        if self.is_tau:
            ...
            color = "black"
        self.fig_arrow = graph.draw_line(self.source.location,
                                         self.target.location,
                                         width=5, color=color)
        tip_cor = (self.source.location[0]/4 + self.target.location[0]*3/4,
                   self.source.location[1]/4 + self.target.location[1]*3/4)
        self.fig_tip = graph.draw_circle(tip_cor, 10,
                                         fill_color=color, line_color=color)
        graph.send_figure_to_back(self.fig_arrow)
        graph.send_figure_to_back(self.fig_tip)

    def delete(self) -> None:
        """Delete figure from our graph object.
        NOTE: This doesn't touch the global list `arrows`.
        """
        graph.delete_figure(self.fig_arrow)
...
hom_layout = [[sg.Text(hom_text1)],
              ...
              [sg.Col(hom_input_col),
               ...
               sg.Text("dim_k Hom(X,Y) = ", key="-HOM-OUT-")],
              [sg.Text(hom_text2)],
              [sg.Text("The radical series of Hom(X,-) is"),
               sg.Listbox([], size=(50, 5), expand_x=True, key="-HOM-COV-"),
               sg.Text("The radical series of Hom(-,Y) is"),
               sg.Listbox([], size=(50, 5), expand_x=True, key="-HOM-CONT-")]
              ]
...
alg_text = """
--- On the computation of dim Hom(X,Y) ---
...
layout = [[sg.Menu(menu_def)],
          [sg.TabGroup([[sg.Tab("Input your AR quiver", input_layout),
                         sg.Tab("Calculation of Hom", hom_layout,
                                key="-HOM-TAB-", disabled=True),
                         sg.Tab("Assumption", [[sg.Text(assump_text)]]),
                         sg.Tab("Algorithm", [[sg.Text(alg_text)]]),
                         sg.Tab("References", [[sg.Text(ref_text)]]),
                         ]],
                       expand_x=True, expand_y=True)]]

window = sg.Window('AR quiver calculator ver. 0.1.0',
...
def draw_vertex(name: str, location: tuple[float, float]) -> None:
    ...
    add it to `vertices`, and return it.
    """
...
def make_info_window() -> None:
    """
    Make the information window (modal), and wait for events.
    """
    info_text = """
    AR quiver calculator ver 0.1.0.
    ...
    info_window = sg.Window('Infomation', layout, modal=True)
    while True:
        event, _ = info_window.read()  # type: ignore
    ...
    if event == sg.WIN_CLOSED:
        ...
        if ((values["-ARROW-"] or values["-TAU-"])
           and not drag_figures and first_time):
            ...
            if search:
                vertex = search[0]
                if not drawning_arrow:
                    # Then this vertex is selected as a source.
                    source_vtx = vertex
                    drawning_arrow = True
                    # Change color!
                    graph.TKCanvas.itemconfig(
                        source_vtx.circle, fill="sandybrown")  # type: ignore
                else:
            ...
            search_arrow = [a for a in arrows
                            if figure in (a.fig_tip, a.fig_arrow)]
            if search_vtx:
                vertex = search_vtx[0]
                delete_vtx(vertex)
            elif search_arrow:
        ...
        try:
            ...
            window["-HOM-TAB-"].update(disabled=False)
            names = [v.name for v in vertices]
            window["-HOM-X-"].update(values=names)
            window["-HOM-Y-"].update(values=names)
        except ValueError as e:
            sg.popup(e)

    elif event == "Save":
        file_path: Optional[str]
        file_path = sg.popup_get_file("Enter a filename.",
        ...
        file_path = sg.popup_get_file("Choose saved data "
        ...
        if X not in names:
            sg.popup("This X is not a vertex!")
        if Y not in names:
            sg.popup("This Y is not a vertex!")
        else:
            try:
                dim = quiver.hom(X, Y)
                window["-HOM-OUT-"].update("dim_k Hom(X,Y) = " + str(dim))
            ...
            for line in needed_part:
                ...
                if scope_count == 0:
                    ...
                    if match:
                        ...
                        loaded_vertices.append(vtx_name)
                elif scope_count == 2:
